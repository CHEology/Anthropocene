...
import {
  ...
  buildRaidFeatures,
  buildTradeFeatures,
  estimateActionReward,
  serializePolicyModule,
  terrainAridity,
  terrainRuggedness,
  trainDecisionPolicy,
  type DecisionAction,
  type TrainingSample,
} from '../src/sim/policy.js';
import type { SimulationConfig, TileState, TribeState, WorldState } from '../src/sim/types.js';
...
function foodCapacityCollapse(tile: TileState) {
  return clamp(
    1 - getFoodCapacity(tile) / Math.max(tile.baseCarryingCapacity.hunt + tile.baseCarryingCapacity.agri, 1),
    0,
    1.2,
  );
}

function storytellerCrisisSignal(state: WorldState) {
  return clamp(
    Math.max(0, 0.55 - state.storyteller.prosperity) * 1.15 +
      state.storyteller.crisisStreak / 18 +
      Math.max(0, state.storyteller.disasterMultiplier - 1) * 0.28,
    0,
    1.2,
  );
}
...
function currentRisk(tribe: TribeState, tile: TileState) {
  return (
    sumDisasterSeverity(tile) +
...
function stageGap(left: TribeState, right: TribeState) {
  const order = {
    foraging: 0,
    tending: 1,
    cultivation: 2,
  ...
  return Math.abs(
    order[left.development.agricultureStage] - order[right.development.agricultureStage],
  ) / 4;
}

function buildPairKeys(state: WorldState) {
  ...
  const pairs: Array<{ left: TribeState; right: TribeState; sharedTile: boolean }> = [];
...
function collectSamples(state: WorldState, prngSeed: number) {
  ...
  for (const tribe of state.tribes) {
    ...
    let bestMigration: TrainingSample | null = null;
    ...
    for (const neighborId of tile.neighbors) {
      ...
      const alliedPresence = clamp(
        tribe.alliances.filter((allyId) => state.tribes.some((candidate) => candidate.id === allyId && candidate.tileId === neighbor.id)).length / 2,
        0,
        1,
      );
      const hostilePresence = clamp(
        state.tribes.filter((candidate) => candidate.tileId === neighbor.id && getRelationship(tribe, candidate.id) < -0.2).length / 3,
        0,
        1,
      );
  ...
  for (const pair of buildPairKeys(state)) {
    ...
    const complementarity = clamp(
      Math.abs(pair.left.abilities.agriculture.current - pair.right.abilities.agriculture.current) / 110 +
        Math.abs(pair.left.abilities.waterEngineering.current - pair.right.abilities.waterEngineering.current) / 150 +
        stageGap(pair.left, pair.right) * 0.18,
      0,
      1.2,
    );
    ...
    const exchangePotential = clamp(
      (surplusLeft * needRight + surplusRight * needLeft) * (pair.sharedTile ? 1 : 0.78) * (0.6 + complementarity),
      0,
      1.2,
    );
    const hostility = clamp(
      0.14 +
        ...
        (pair.left.exchange.raidExposure + pair.right.exchange.raidExposure) * 0.08,
      0,
      1.2,
    );
    ...
    const leftStrength = clamp(
      Math.sqrt(Math.max(pair.left.pop, 1)) * (0.85 + pair.left.abilities.attack.current / 90) * (0.88 + pair.left.abilities.organization.current / 120) / 16,
      0,
      1.4,
    );
    const rightStrength = clamp(
      Math.sqrt(Math.max(pair.right.pop, 1)) * (0.85 + pair.right.abilities.attack.current / 90) * (0.88 + pair.right.abilities.organization.current / 120) / 16,
      0,
      1.4,
    );
    ...
    const raidFeatures = buildRaidFeatures({
      ...
      relation: clamp((relation + 1) / 2, 0, 1.2),
      strengthEdge,
      exchangePotential: clamp(
        (pair.left.exchange.tradeVolume + pair.right.exchange.tradeVolume) * 0.4 +
          (pair.left.development.domestication + pair.right.development.domestication) / 200,
        0,
        1.2,
      ),
...
function buildConfig(seed: number, worldPreset: SimulationConfig['worldPreset'], overrides?: Partial<SimulationConfig['globals']>) {
...
async function main() {
  ...
  const aggregateCounts = new Map<DecisionAction, number>();

  for (const run of TRAINING_RUNS) {
    const engine = createSimulationEngine(run.config, { policy: null });
    const runCounts = new Map<DecisionAction, number>();
