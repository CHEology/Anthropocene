...
function finiteNumberOrThrow(value: unknown, message: string, min: number, max: number) {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < min || value > max) {
    throw new InvariantError(message, { value });
  }
}

function safeIntegerOrThrow(value: unknown, message: string, min: number, max: number) {
...
export function sanitizeSimulationConfig(
  input?: DeepPartial<SimulationConfig>,
  baseInput: SimulationConfig = DEFAULT_SIMULATION_CONFIG,
): SimulationConfig {
  ...
  return {
    ...
    runtime: {
      ...
      snapshotThrottleMs: clampInteger(
        runtime.snapshotThrottleMs,
        16,
        2_000,
        base.runtime.snapshotThrottleMs,
      ),
    },
    enabledSystems: {
      globalClimate:
        typeof enabledSystems.globalClimate === 'boolean'
          ? enabledSystems.globalClimate
          : base.enabledSystems.globalClimate,
      tileRecovery:
        typeof enabledSystems.tileRecovery === 'boolean'
          ? enabledSystems.tileRecovery
          : base.enabledSystems.tileRecovery,
      tribeDynamics:
        typeof enabledSystems.tribeDynamics === 'boolean'
          ? enabledSystems.tribeDynamics
          : base.enabledSystems.tribeDynamics,
      interventions:
        typeof enabledSystems.interventions === 'boolean'
...
export function sanitizeStepYears(years: unknown, currentYear: number) {
  if (years === undefined) {
    return 1;
  }
  ...
  if (wholeYears > SERVER_LIMITS.maxYearsPerRequest) {
    throw new ApiError(
      ...
      `A single request may advance at most ${SERVER_LIMITS.maxYearsPerRequest} years.`,
    );
  }
  ...
  if (currentYear + wholeYears > SERVER_LIMITS.maxYearsPerSession) {
    throw new ApiError(
      ...
      `This session cannot advance past year ${SERVER_LIMITS.maxYearsPerSession}.`,
    );
  }
...
export function sanitizeInterventionInput(
  input: unknown,
  currentYear: number,
): InterventionCommand {
  if (!isRecord(input)) {
  ...
  const kind =
    input.kind === 'climate-pulse' || input.kind === 'observation-note'
      ? input.kind
      : undefined;
  ...
  if (!kind) {
    throw new ApiError(
      ...
      'Intervention kind must be climate-pulse or observation-note.',
    );
  }

  const scheduledYear = clampInteger(
    input.scheduledYear,
    currentYear,
  ...
  const label = sanitizeString(
    input.label,
    SERVER_LIMITS.maxLabelLength,
    kind === 'climate-pulse' ? 'Climate pulse' : 'Observation note',
  );
  ...
  if (kind === 'climate-pulse') {
    return {
      ...
      scheduledYear,
      payload: {
        temperatureDelta: clampFiniteNumber(
          payload.temperatureDelta,
          -SERVER_LIMITS.maxClimatePulseDelta,
          SERVER_LIMITS.maxClimatePulseDelta,
          0,
        ),
        duration: clampInteger(
          payload.duration,
          1,
          SERVER_LIMITS.maxClimatePulseDuration,
          120,
        ),
      },
    };
...
export function trimStateForTransport(state: WorldState): WorldState {
  return {
...
export function assertWorldStateInvariants(state: WorldState) {
  safeIntegerOrThrow(
    state.year,
    'World year is outside the supported simulation horizon.',
    0,
    SERVER_LIMITS.maxYearsPerSession,
  );
  safeIntegerOrThrow(
    state.seed,
    'World seed is not a supported 32-bit integer.',
    1,
    2_147_483_647,
  );
  ...
  finiteNumberOrThrow(
    state.globalClimate.baseline,
    'Global climate baseline must remain finite and bounded.',
    SERVER_LIMITS.minTemperature,
    SERVER_LIMITS.maxTemperature,
  );
  finiteNumberOrThrow(
    state.globalClimate.anomaly,
    'Global climate anomaly must remain finite and bounded.',
    -SERVER_LIMITS.maxClimatePulseDelta * 4,
    SERVER_LIMITS.maxClimatePulseDelta * 4,
  );
  finiteNumberOrThrow(
    state.globalClimate.meanTemperature,
    'Global mean temperature must remain finite and bounded.',
    SERVER_LIMITS.minTemperature,
    SERVER_LIMITS.maxTemperature,
  );
  safeIntegerOrThrow(
    state.metrics.totalPopulation,
    'World total population must remain within backend limits.',
    0,
    SERVER_LIMITS.maxMetricMagnitude,
  );
  safeIntegerOrThrow(
    state.metrics.tribeCount,
    'World tribe counts must remain within backend limits.',
    0,
    SERVER_LIMITS.maxTribes,
  );
  safeIntegerOrThrow(
    state.metrics.innovations,
    'World innovation counts must remain within backend limits.',
    0,
    SERVER_LIMITS.maxMetricMagnitude,
  );
  safeIntegerOrThrow(
    state.metrics.conflicts,
    'World conflict counts must remain within backend limits.',
    0,
    SERVER_LIMITS.maxMetricMagnitude,
  );
  finiteNumberOrThrow(
    state.metrics.averageComfort,
    'World mean comfort must remain finite and bounded.',
    0,
    SERVER_LIMITS.maxMeanComfort,
  );
  finiteNumberOrThrow(
    state.metrics.averagePressure,
    'World mean pressure must remain finite and bounded.',
    0,
  ...
  for (const tile of state.tiles) {
    ...
    finiteNumberOrThrow(
      tile.water,
      'Tile water value must remain finite and bounded.',
      0,
      SERVER_LIMITS.maxCapacity,
    );
    finiteNumberOrThrow(
      tile.temperature,
      'Tile temperature value must remain finite and bounded.',
      SERVER_LIMITS.minTemperature,
      SERVER_LIMITS.maxTemperature,
    );
    finiteNumberOrThrow(
      tile.comfort,
      'Tile comfort value must remain finite and bounded.',
      0,
      SERVER_LIMITS.maxMeanComfort,
    );
    finiteNumberOrThrow(
    ...
    finiteNumberOrThrow(
      ...
      'Tile agricultural capacity must remain finite and bounded.',
      0,
      SERVER_LIMITS.maxCapacity,
    );
    finiteNumberOrThrow(
      tile.carryingCapacity.water,
      'Tile water capacity must remain finite and bounded.',
      0,
      SERVER_LIMITS.maxCapacity,
    );
  ...
  for (const tribe of state.tribes) {
    ...
    safeIntegerOrThrow(
      tribe.pop,
      'Tribe population must remain a safe integer within backend limits.',
      0,
      SERVER_LIMITS.maxPopulation,
    );
    finiteNumberOrThrow(
      tribe.pressures.total,
      'Tribe total pressure must remain finite and bounded.',
      0,
      SERVER_LIMITS.maxPressure,
    );
  ...
  for (const point of state.history) {
    safeIntegerOrThrow(
      point.year,
      'History points must use safe integer years.',
      0,
      SERVER_LIMITS.maxYearsPerSession,
    );
    safeIntegerOrThrow(
      point.totalPopulation,
      'History population values must remain within backend limits.',
      0,
      SERVER_LIMITS.maxMetricMagnitude,
    );
    safeIntegerOrThrow(
      point.tribeCount,
      'History tribe counts must remain within backend limits.',
      0,
      SERVER_LIMITS.maxTribes,
    );
    safeIntegerOrThrow(
      point.innovations,
      'History innovation counts must remain within backend limits.',
      0,
      SERVER_LIMITS.maxMetricMagnitude,
    );
    safeIntegerOrThrow(
      point.conflicts,
      'History conflict counts must remain within backend limits.',
      0,
      SERVER_LIMITS.maxMetricMagnitude,
    );
  }
