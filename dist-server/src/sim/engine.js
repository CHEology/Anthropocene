...
function alliedPresenceOnTile(tribe, tileId, tribes) {
    return clamp(tribe.alliances.reduce((sum, allyId) => {
        return sum + (tribes.some((candidate) => candidate.id === allyId && candidate.tileId === tileId) ? 1 : 0);
    }, 0), 0, 3);
}
function hostilePresenceOnTile(tribe, tileId, tribes) {
    return clamp(tribes.reduce((sum, candidate) => {
...
function computeMetrics(state, previousMetrics, events) {
    ...
    const averageComfort = state.tiles.reduce((sum, tile) => sum + tile.comfort, 0) / Math.max(state.tiles.length, 1);
    const averagePressure = state.tribes.reduce((sum, tribe) => sum + tribe.pressures.total, 0) /
        Math.max(state.tribes.length, 1);
    const averageFoodStores = state.tribes.reduce((sum, tribe) => sum + tribe.foodStores, 0) / Math.max(state.tribes.length, 1);
    const averageGeneticDiversity = state.tribes.reduce((sum, tribe) => sum + tribe.geneticDiversity, 0) / Math.max(state.tribes.length, 1);
    const averageMegafauna = state.tiles.reduce((sum, tile) => sum + tile.megafaunaIndex, 0) / Math.max(state.tiles.length, 1);
    ...
    return {
        totalPopulation,
        tribeCount,
        innovations: previousMetrics.innovations + events.filter((event) => event.kind === 'innovation').length,
        conflicts: previousMetrics.conflicts +
...
function applyTilePhase(read, write, config, prng, events, changedTileIds) {
    ...
    for (const tile of write.tiles) {
        ...
        const huntRecovery = Math.min(previous.baseCarryingCapacity.hunt * 0.02 * recoveryMod, previous.baseCarryingCapacity.hunt - previous.carryingCapacity.hunt);
        const agriRecovery = Math.min(previous.baseCarryingCapacity.agri * 0.015 * recoveryMod, previous.baseCarryingCapacity.agri - previous.carryingCapacity.agri);
        ...
        const spreadChance = neighborPlaguePressure > 0 && totalPopulation > 0
            ? clamp(neighborPlaguePressure * (0.016 + tradeIntensity * 0.04 + settlement * 0.03), 0, 0.18)
            : 0;
...
function computeTribeResourceContext(tribe, tile, tilePopulation) {
    ...
    const crowdingForagePenalty = 1 - clamp(Math.max(0, huntDensity - (0.62 + stageProfile.targetSedentism * 0.15)) * (0.32 + mobility * 0.4), 0, 0.82);
    const crowdingFarmPenalty = 1 - clamp(Math.max(0, foodDensity - (0.92 + stageProfile.targetSedentism * 0.12)) * (0.1 + stageProfile.targetSedentism * 0.18), 0, 0.36);
    const effectiveMegafauna = tile.megafaunaIndex < 0.1 ? 0 : tile.megafaunaIndex;
    const megafaunaBonus = 1 + effectiveMegafauna * 0.5;
    ...
    const competition = clamp((foodDensity - crowdingThreshold) *
        ...
        Math.max(0, tilePopulation / Math.max(tile.baseCarryingCapacity.hunt + tile.baseCarryingCapacity.agri, 1) - 1.05) * 0.1, 0, 1);
    ...
    const health = clamp(plagueBurden * 0.62 +
        disasterBurden * 0.12 +
        ...
        (leaderModifiers.plagueResilience - 1) * 0.18, 0, 1);
    const carryingCapacity = round(clamp(Math.min(effectiveFoodWithStores, supportedByWater + effectiveStoredFood * 0.12) +
        ...
        competition * 14 -
        resourceCollapse * 12 -
        geneticRisk * 8, MIN_TRIBE_POPULATION * 2, MAX_TRIBE_POPULATION), 2);
...
function applyTribePhase(read, write, config, prng, events, changedTribeIds, changedTileIds, populationCarry, decisionPolicy) {
    ...
    for (const tribeId of orderedIds) {
        ...
        const innovationProbability = config.globals.G_innovation *
        ...
        const progressDelta = (agriSuitability * 0.48 +
            ...
            1.08 *
            intensifyAdjustment;
        target.development.domestication = round(clamp(source.development.domestication + progressDelta, 0, 100), 2);
        ...
        const foodMultiplier = clamp(1 - target.pressures.food * 0.78 + source.exchange.tradeVolume * 0.08, 0.08, 1.25);
        const comfortMultiplier = clamp(1 - (target.pressures.heat + target.pressures.cold) * 0.3 - target.pressures.health * 0.12, 0.42, 1.12);
        ...
        const rBase = clamp(config.globals.G_birth *
            ...
            resourceContext.disasterBurden * 0.01, -0.1, 0.04);
        ...
        const pressureDeathRate = clamp(Math.max(0, target.pressures.food - 0.38) * 0.034 +
            Math.max(0, target.pressures.water - 0.4) * 0.04 +
            source.exchange.raidExposure * 0.022 +
            source.exchange.warExhaustion * 0.018 +
            resourceContext.geneticRisk * 0.01, 0, 0.075) * starvationDamper;
        ...
        target.foodStores = round(clamp(source.foodStores + storesGain - storesDraw, 0, 1), 3);
        ...
        const contactBuffer = clamp(source.alliances.length * 0.06 + source.exchange.tradeVolume * 0.18 + source.exchange.diffusion * 0.1, 0, 0.45);
    ...
    for (const [tileId, load] of tileLoads) {
        ...
        const huntDepletion = load.foraging * (0.0056 + load.mobility * 0.00026) +
            Math.max(0, huntPressure - 0.78) * tile.carryingCapacity.hunt * (0.016 + load.crowding * 0.002);
        const agriDepletion = load.farming * 0.0042 + Math.max(0, agriPressure - 1) * tile.carryingCapacity.agri * 0.009;
...
function applyInteractionPhase(read, write, config, prng, events, changedTribeIds, decisionPolicy) {
    ...
    for (const pair of pairs) {
        ...
        const hostility = config.globals.G_hostility * 0.42 +
        ...
        const aggressionLeft = readLeft.pressures.food * 0.48 +
        ...
        const aggressionRight = readRight.pressures.food * 0.48 +
...
function applyMigrationPhase(read, write, config, prng, events, changedTribeIds, decisionPolicy) {
    ...
    for (const tribe of read.tribes) {
        ...
        const defeatVulnerability = clamp(tribe.exchange.raidExposure * 0.55 +
            tribe.exchange.warExhaustion * 0.4 +
            Math.max(0, 0.4 - tribe.foodStores) * 0.4, 0, 1.2);
        const migrationPressure = Math.max(tribe.pressures.total, tribe.pressures.food * 1.26, tribe.pressures.water * 1.18, tribe.pressures.health * 1.08, tribe.pressures.competition * 1.28, currentRisk * 0.82, defeatVulnerability * 0.74);
        const mobility = mobilityProfile(tribe);
        const frontierDrive = clamp(tribe.pressures.competition * 1.02 +
            tribe.pressures.food * 0.62 +
            mobility * 0.26 +
            storytellerPressure * 0.12, 0, 1.6);
        ...
        for (const neighborId of currentTile.neighbors) {
            ...
            const neighborRisk = sumDisasterSeverity(neighbor) +
            ...
            let score = features.resourceDelta * 1.28 +
        ...
        const migrationChance = clamp(config.globals.G_migration *
            (0.32 +
            ...
            migrationAdjustment /
            (1 + stageProfile.migrationFriction * 0.92 + tribe.development.sedentism * 0.78), 0.03, 0.94);
        const acceptanceThreshold = -0.46 + frontierDrive * 0.22 + mobility * 0.1 - storytellerPressure * 0.08;
        if (bestTile.id !== currentTile.id && bestScore > acceptanceThreshold && prng.next() < migrationChance) {
            ...
            target.development.domestication = round(clamp(target.development.domestication -
                ...
                stageRank(target.development.agricultureStage) * 0.4, 0, 100), 2);
            ...
            events.push({
                ...
                detail: currentRisk > 0.45 || tribe.pressures.competition > 0.42
...
function applyFissionPhase(read, write, config, prng, events, changedTribeIds, populationCarry) {
    ...
    for (const source of read.tribes) {
        ...
        const splitPressure = clamp((source.pop - 150) / 260 +
            ...
            target.development.sedentism * 0.08, 0, 0.85);
...
function updateStoryteller(state, events) {
    ...
    const significantEvents = events.filter((event) => event.kind === 'disaster' ||
        event.kind === 'disease' ||
        event.kind === 'combat' ||
...
export function createSimulationEngine(initialConfig, options = {}) {
    ...
    function stepOnce() {
        ...
        if (config.enabledSystems.tribeDynamics) {
            applyTribePhase(current, staging, config, prng, emittedEvents, changedTribeIds, changedTileIds, populationCarry, decisionPolicy);
        }
        current = staging;
        staging = cloneState(current);
        ...
        current = staging;
        staging = cloneState(current);
        applyFissionPhase(current, staging, config, prng, emittedEvents, changedTribeIds, populationCarry);
        current = staging;
