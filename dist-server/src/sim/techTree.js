...
export const ALL_TECH_IDS = Object.keys(TECH_TREE);
export function hasTech(knownTechs, techId) {
    return knownTechs.includes(techId);
}
export function hasAllPrerequisites(knownTechs, techId) {
    const tech = TECH_TREE[techId];
    return tech.prerequisites.every((prereq) => knownTechs.includes(prereq));
}
export function getDiscoverableTechs(knownTechs) {
    return ALL_TECH_IDS.filter((techId) => !knownTechs.includes(techId) && hasAllPrerequisites(knownTechs, techId));
}
export function getTechAbilityBonuses(knownTechs) {
    const bonuses = {
        foraging: 0,
        agriculture: 0,
        heatTolerance: 0,
...
export function getTechHealthBonus(knownTechs) {
    let bonus = 0;
    if (knownTechs.includes('basic-medicine'))
        bonus += 0.06;
    if (knownTechs.includes('well-digging'))
        bonus += 0.03;
    if (knownTechs.includes('canal-building'))
        bonus += 0.02;
    return bonus;
}
export function getTechFoodStorageBonus(knownTechs) {
    let bonus = 0;
    if (knownTechs.includes('pottery'))
        bonus += 0.04;
    if (knownTechs.includes('granary-storage'))
        bonus += 0.08;
    if (knownTechs.includes('fermentation'))
        bonus += 0.03;
    return bonus;
}
export const STARTING_TECHS_FORAGER = [
    'fire-mastery',
    'animal-tracking',
    'oral-tradition',
    'water-finding',
];
export const STARTING_TECHS_TENDING = [
    ...STARTING_TECHS_FORAGER,
    'cooking',
    'hafted-tools',
    'tanning',
];
export const STARTING_TECHS_CULTIVATION = [
...
export const STARTING_TECHS_AGROPASTORAL = [
