...
function sendBytes(response, statusCode, contentType, body) {
    response.statusCode = statusCode;
    setCorsHeaders(response);
...
function selectPayload(body, key) {
...
async function serveStatic(pathname, response) {
    ...
    if (file) {
        sendBytes(response, 200, MIME_TYPES[extname(absolutePath)] ?? 'application/octet-stream', file);
        return true;
    }
    if (!wantsHtml) {
        return false;
...
async function handleApiRequest(request, response, service, pathname) {
    ...
    if (request.method === 'POST' && pathname === '/api/simulations') {
        ...
        sendJson(response, 201, { session: service.createSession(config) });
        return true;
    }
    ...
    if (request.method === 'POST' && segments.length === 4 && segments[3] === 'reset') {
        const body = await readJsonBody(request);
        const config = selectPayload(body, 'config');
        sendJson(response, 200, { session: service.resetSession(sessionId, config) });
        return true;
    }
    throw new ApiError(404, 'not_found', 'API route not found.');
}
export async function handleApiHttpRequest(request, response, service) {
