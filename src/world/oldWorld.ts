...
interface TileSeed {
  ...
  coastal?: boolean;
  movementTags?: TileState['movementTags'];
}
...
function defaultMovementTags(terrain: TileState['terrain'], coastal: boolean): TileState['movementTags'] {
  const tags: TileState['movementTags'] = [];
...
function defaultStartingTechs(stage: TribeState['development']['agricultureStage']): TechnologyId[] {
  switch (stage) {
    case 'agropastoral':
...
function isRiverValley(lon: number, lat: number) {
  return (
    inEllipse(lon, lat, 31, 25, 3.4, 9.8) ||
...
function classifyDetailedTerrain(tile: DetailedTileDraft): TileState['terrain'] {
  ...
  if (
    inBox(tile.lon, tile.lat, -2, 31, -5, 7) ||
    tile.lat >= 55 ||
  ...
  if (
    inBox(tile.lon, tile.lat, -18, 40, -18, 16) ||
...
const AUTHORED_OLD_WORLD_TERRAIN_MAP: Record<string, TileState['terrain']> = {
  r: 'river_valley',
  s: 'savanna',
  c: 'coast',
  f: 'forest',
...
function buildAuthoredMovementTags(
  tile: DetailedTileDraft,
  terrain: TileState['terrain'],
): TileState['movementTags'] {
  ...
  if (
    (terrain === 'mountain' || terrain === 'highland') &&
...
function generateDetailedEurasiaPreset(): WorldPresetDefinition {
  ...
  const draftTiles: Array<DetailedTileDraft & { terrain: TileState['terrain'] }> = [];
  ...
  const tileNameById = new Map(tileSeeds.map((tile) => [tile.id, tile.name] as const));
  const tileSeedById = new Map(tileSeeds.map((tile) => [tile.id, tile] as const));
  const neighborsById = new Map(
    tileSeeds.map((tile) => [tile.id, buildNeighbors(tileSeeds, tile.id, tile.q, tile.r)] as const),
  );
  const pickTileId = createNearestTilePicker(
    annotatedTiles.map((tile) => ({ id: tile.seed.id, lon: tile.lon, lat: tile.lat })),
  );
  const pickRiverTileId = createNearestTilePicker(
    annotatedTiles
  ...
  function findTilePath(
    startId: string,
    endId: string,
    predicate: (tile: TileSeed) => boolean,
  ) {
    if (startId === endId) {
      return [startId];
    }
  ...
  function stitchWaypointPath(
    waypointTileIds: string[],
    predicate: (tile: TileSeed) => boolean,
  ) {
    const dedupedWaypoints = waypointTileIds.filter(
      (tileId, index) => index === 0 || tileId !== waypointTileIds[index - 1],
    );
  ...
  function buildRiverLane(
    ...
    const tileIds = stitchWaypointPath(
      waypoints.map(([lon, lat]) => pickRiverTileId(lon, lat)),
      (tile) => tile.terrain === 'river_valley',
    );
    return {
      id,
      label,
